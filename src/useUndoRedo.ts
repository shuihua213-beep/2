...
import { AppState, HistoryState, HistoryAction, historyReducer, createInitialHistoryState, GameState, Upgrades } from './historyReducer';
...
function loadHistory(defaultInitial: AppState): HistoryState {
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.present && parsed.present.gameState && parsed.present.upgrades && Array.isArray(parsed.past) && Array.isArray(parsed.future)) {
        return parsed;
      }
    }
...
export function useUndoRedo(initial: AppState) {
  ...
  const dispatch = useCallback((action: HistoryAction) => {
    baseDispatch(action);
    if (batchDepthRef.current === 0 && (action.type === 'SET_GAME_STATE' || action.type === 'SET_UPGRADES')) {
