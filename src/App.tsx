...
export default function App() {
    ...
    workerRef.current.onmessage = (e: MessageEvent<MainThreadMessage>) => {
      ...
      switch (message.type) {
        case 'update':
          ...
          setCollectibles(message.collectibles);
          setBullets(message.bullets);
          setParticles(message.particles);
          setGameState(message.gameState);
          setCamelFrame(message.camelFrame);
          setBackgroundOffset(message.backgroundOffset);
          break;
        case 'gameOver':
          setGameState(message.gameState);
          break;
  ...
  const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (workerRef.current) {
      workerRef.current.postMessage({ type: 'shoot', targetX: x, targetY: y } as WorkerMessage);
    }
  };
  ...
  const startGame = () => {
    ...
    if (workerRef.current) {
      workerRef.current.postMessage({ type: 'startGame', gameState: newGameState, upgrades } as WorkerMessage);
    }
  };
